...
from src.splitting import make_assignments, check_assignments, development_cv, nearest_date_boundary, run_validation
    ...
    def test_frozen_artifacts_reject_plan_changes_without_overwriting(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch('src.splitting.METADATA_SHA256', digest):
                _, plan = run_validation(processed, output)
                plan_path = output / 'validation_split_plan.json'
                ...
                with self.assertRaisesRegex(ValueError, 'Frozen plan differs'):
                    run_validation(processed, output)
