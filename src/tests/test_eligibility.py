...
from src.eligibility import prepare_records, preserve_csv


class EligibilityTests(unittest.TestCase):
    def test_renamed_compressed_csv_preserves_original_bytes(self):
        with tempfile.TemporaryDirectory() as folder:
            path = Path(folder) / 'eligibility_metadata.csv.gz'
            frame = pd.DataFrame({'value': [1, 2]})
            with gzip.GzipFile(filename='old_filename.csv', mode='wb', fileobj=path.open('wb'), mtime=0) as out:
                out.write(frame.to_csv(index=False, lineterminator='\n').encode())
            original = path.read_bytes()
            preserve_csv(frame, path)
            self.assertEqual(path.read_bytes(), original)

    def test_changed_frozen_csv_is_rejected(self):
        with tempfile.TemporaryDirectory() as folder:
            path = Path(folder) / 'frozen.csv.gz'
            preserve_csv(pd.DataFrame({'value': [1]}), path)
            original = path.read_bytes()
            with self.assertRaisesRegex(ValueError, 'Frozen CSV content differs'):
                preserve_csv(pd.DataFrame({'value': [2]}), path)
            self.assertEqual(path.read_bytes(), original)
