...
def main():
    for name in ['evaluation/evaluation_summary.json', 'test_comparison/comparison_summary.json']:
        record = json.loads((ROOT / 'data/processed/results' / name).read_text())
        for relative, expected in record['output_sha256'].items():
            assert hashlib.sha256((ROOT / relative).read_bytes()).hexdigest() == expected, relative
    lr = pd.concat([pd.read_csv(ROOT / f'data/processed/results/evaluation/test_predictions_{i:02d}.csv.gz')
                    for i in range(1, 5)], ignore_index=True)
    rf = pd.read_csv(ROOT / 'data/processed/results/test_comparison/random_forest_test_probabilities.csv.gz')
    assert np.array_equal(lr.source_row_id, rf.source_row_id)
    assert np.array_equal(lr.actual, rf.actual)
    coefficients = pd.read_csv(ROOT / 'data/processed/results/evaluation/feature_coefficients.csv').head(6)
