...
from scripts.verify_notebooks import NOTEBOOKS, run_verification
...
class ReproductionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.root.mkdir()
        for folder in ('data/processed', 'data/splits', 'src', 'models',
                       'data/results/step12', 'data/results/step13', 'data/results/step15'):
    ...
    def test_workspace_is_external_and_independent(self):
        ...
        self.assertFalse((run / 'data/results/step13').exists())
    ...
    def test_windows_newlines_and_path_keys_are_not_score_drift(self):
        run = self.start()
        self.make_evidence(run)
        protocol = run / 'data/results/step12/search_protocol.json'
        protocol.write_bytes(protocol.read_bytes().replace(b'\n', b'\r\n'))
        summary_path = run / 'data/results/step12/tuning_summary.json'
    ...
    def test_candidate_membership_change_rejected(self):
        run = self.start()
        self.make_evidence(run)
        table = run / 'data/results/step12/candidate_results.csv'
    ...
    def test_runner_separates_execution_from_numerical_status(self):
        ...
        with patch('scripts.verify_notebooks.environment_record', return_value={}), \
             patch('scripts.verify_notebooks.collect_comparison', return_value=comparison), \
             patch('scripts.verify_notebooks.subprocess.run') as process, \
             patch('scripts.verify_notebooks.tempfile.mkdtemp',
